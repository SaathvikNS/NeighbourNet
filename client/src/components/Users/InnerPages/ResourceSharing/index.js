...
const ResourceSharing = () => {
    ...
    const fetchData = async () => {
        try{
            const fetchResponse = await axios.get(`${api}/actionresponse/get-resource-requests/${userid}`);
    ...
    const handleDelete = async (id) => {
        try{
            const deleteResponse = await axios.delete(`${api}/actions/delete-resource/${id}`)
            console.log(deleteResponse.data)
            setSnackbarMessage('Resource Request Deleted Successfully');
    ...
    const handleresponse = async (id) => {
        try{
            const response = await axios.post(`${api}/actions/resource-response`, {id, userid})
  ...
  return (
    <div>
        <Box sx={{ backgroundColor: '#1E201E', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', width: '100%'}}>
            ...
            <Fab onClick={handleResourceRequest} size='small' sx={{position: 'fixed', bottom: 0, right: 0, margin: '2rem' }}>
                <AddIcon />
            </Fab>
            </Box>
        </Box>
